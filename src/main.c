...
static void mainloop(void) {
    ...
    // draw ghost targets
    vec2 rt = red_target(p);
    vec2 screen_rt = tile_to_screen(rt);
    draw_target(renderer, screen_rt, 255, 0, 0);

    draw_red_path(p, renderer); 
    // vec2 rnp = red_next_pos(p);
    // vec2 screen_rnp = tile_to_screen(rnp);
    // draw_target(renderer, screen_rnp, 255, 0, 0);

    vec2 pt = pink_target(p);
    vec2 screen_pt = tile_to_screen(pt);
    draw_target(renderer, screen_pt, 255, 0, 255);
    vec2 ct = clyde_target(p);
    vec2 screen_ct = tile_to_screen(ct);
    draw_target(renderer, screen_ct, 255, 255, 0);
    vec2 bt = blue_target(p);
    vec2 screen_bt = tile_to_screen(bt);
    draw_target(renderer, screen_bt, 0, 0, 255);
